...
/// <summary>
///     VK_EXT_transform_feedback - device extension (nr. 29) - author 'NV' [platform '' | contact 'Piers Daniell @pdaniell-nv']
///     <br />
...
public static unsafe class VkExtTransformFeedback
{
    ...
    /// <summary> An UTF8 null terminated version of <see cref="VK_EXT_TRANSFORM_FEEDBACK_EXTENSION_NAME" /> represented by an UTF16 string. </summary>
    /// <remarks>
    ///     Example usage:<br />
    ///     <br />
    ...
    /// <param name="firstBinding"> firstBinding is the index of the first transform feedback binding whose state is updated by the command. </param>
    /// <param name="bindingCount"> bindingCount is the number of transform feedback bindings whose state is updated by the command. </param>
    ...
    /// <param name="pSizes"> pSizes is NULL or a pointer to an array of VkDeviceSizebuffer sizes, specifying the maximum number of bytes to capture to the corresponding transform feedback buffer. If pSizes is NULL, or the value of the pSizes array element is VK_WHOLE_SIZE, then the maximum number of bytes captured will be the size of the corresponding buffer minus the buffer offset. </param>
    ...
    /// </summary>
    /// <param name="commandBuffer"> commandBuffer is the command buffer into which the command is recorded. </param>
    /// <param name="firstCounterBuffer"> firstCounterBuffer is the index of the first transform feedback buffer corresponding to pCounterBuffers[0] and pCounterBufferOffsets[0]. </param>
    /// <param name="counterBufferCount"> counterBufferCount is the size of the pCounterBuffers and pCounterBufferOffsets arrays. </param>
    /// <param name="pCounterBuffers"> pCounterBuffers is NULL or a pointer to an array of VkBuffer handles to counter buffers. Each buffer contains a 4 byte integer value representing the byte offset from the start of the corresponding transform feedback buffer from where to start capturing vertex data. If the byte offset stored to the counter buffer location was done using vkCmdEndTransformFeedbackEXT it can be used to resume transform feedback from the previous location. If pCounterBuffers is NULL, then transform feedback will start capturing vertex data to byte offset zero in all bound transform feedback buffers. For each element of pCounterBuffers that is VK_NULL_HANDLE, transform feedback will start capturing vertex data to byte zero in the corresponding bound transform feedback buffer. </param>
    /// <param name="pCounterBufferOffsets"> pCounterBufferOffsets is NULL or a pointer to an array of VkDeviceSize values specifying offsets within each of the pCounterBuffers where the counter values were previously written. The location in each counter buffer at these offsets must be large enough to contain 4 bytes of data. This data is the number of bytes captured by the previous transform feedback to this buffer. If pCounterBufferOffsets is NULL, then it is assumed the offsets are zero. </param>
    public static readonly delegate*<
        VkCommandBuffer /*commandBuffer*/,
    ...
    /// <param name="firstCounterBuffer"> firstCounterBuffer is the index of the first transform feedback buffer corresponding to pCounterBuffers[0] and pCounterBufferOffsets[0]. </param>
    /// <param name="counterBufferCount"> counterBufferCount is the size of the pCounterBuffers and pCounterBufferOffsets arrays. </param>
    /// <param name="pCounterBuffers"> pCounterBuffers is NULL or a pointer to an array of VkBuffer handles to counter buffers. The counter buffers are used to record the current byte positions of each transform feedback buffer where the next vertex output data would be captured. This can be used by a subsequent vkCmdBeginTransformFeedbackEXTcall to resume transform feedback capture from this position. It can also be used by vkCmdDrawIndirectByteCountEXT to determine the vertex count of the draw call. </param>
    /// <param name="pCounterBufferOffsets"> pCounterBufferOffsets is NULL or a pointer to an array of VkDeviceSize values specifying offsets within each of the pCounterBuffers where the counter values can be written. The location in each counter buffer at these offsets must be large enough to contain 4 bytes of data. The data stored at this location is the byte offset from the start of the transform feedback buffer binding where the next vertex data would be written. If pCounterBufferOffsets is NULL, then it is assumed the offsets are zero. </param>
    public static readonly delegate*<
        VkCommandBuffer /*commandBuffer*/,
        uint /*firstCounterBuffer*/,
        uint /*counterBufferCount*/,
    ...
    /// <param name="index"> index is the query type specific index. When the query type is VK_QUERY_TYPE_TRANSFORM_FEEDBACK_STREAM_EXTor VK_QUERY_TYPE_PRIMITIVES_GENERATED_EXT, the index represents the vertex stream. </param>
    public static readonly delegate*<
        VkCommandBuffer /*commandBuffer*/,
    ...
    /// <param name="counterBufferOffset"> counterBufferOffset is the offset into the buffer used to read the byte count, which is used to calculate the vertex count for this draw call. </param>
    /// <param name="counterOffset"> counterOffset is subtracted from the byte count read from the counterBuffer at the counterBufferOffset </param>
    /// <param name="vertexStride"> vertexStride is the stride in bytes between each element of the vertex data that is used to calculate the vertex count from the counter value. This value is typically the same value that was used in the graphics pipeline state when the transform feedback was captured as the XfbStride. </param>
    public static readonly delegate*<
        VkCommandBuffer /*commandBuffer*/,
        uint /*instanceCount*/,
        uint /*firstInstance*/,
