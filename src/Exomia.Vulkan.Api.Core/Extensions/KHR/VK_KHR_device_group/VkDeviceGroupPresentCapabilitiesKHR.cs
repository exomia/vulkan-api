...
public unsafe struct VkDeviceGroupPresentCapabilitiesKHR
{
    ...
    /// <summary> presentMask is an array of VK_MAX_DEVICE_GROUP_SIZEuint32_t masks, where the mask at element i is non-zero if physical device i has a presentation engine, and where bit jis set in element i if physical device i can present swapchain images from physical device j. If element i is non-zero, then bit i must be set. </summary>
    public VkArray32<uint> presentMask;

    /// <summary> modes is a bitmask of VkDeviceGroupPresentModeFlagBitsKHRindicating which device group presentation modes are supported. </summary>
