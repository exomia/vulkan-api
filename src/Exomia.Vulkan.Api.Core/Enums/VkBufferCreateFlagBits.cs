...
public enum VkBufferCreateFlagBits
{
    ...
    /// <summary> VK_BUFFER_CREATE_SPARSE_RESIDENCY_BIT specifies that the buffer can be partially backed using sparse memory binding. Buffers created with this flag must also be created with the VK_BUFFER_CREATE_SPARSE_BINDING_BIT flag. </summary>
    VK_BUFFER_CREATE_SPARSE_RESIDENCY_BIT = 0x2,

    /// <summary> VK_BUFFER_CREATE_SPARSE_ALIASED_BIT specifies that the buffer will be backed using sparse memory binding with memory ranges that might also simultaneously be backing another buffer (or another portion of the same buffer). Buffers created with this flag must also be created with the VK_BUFFER_CREATE_SPARSE_BINDING_BIT flag. </summary>
    ...
    /// <summary>
    ///     VK_BUFFER_CREATE_DEVICE_ADDRESS_CAPTURE_REPLAY_BIT specifies that the buffer&#8217;s address can be saved and reused on a subsequent run (e.g. for trace capture and replay), see
    ///     <a
