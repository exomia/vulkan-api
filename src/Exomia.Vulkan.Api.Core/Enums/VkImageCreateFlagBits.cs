...
public enum VkImageCreateFlagBits
{
    ...
    /// <summary> VK_IMAGE_CREATE_SPARSE_RESIDENCY_BIT specifies that the image canbe partially backed using sparse memory binding. Images created with this flag must also be created with the VK_IMAGE_CREATE_SPARSE_BINDING_BIT flag. </summary>
    VK_IMAGE_CREATE_SPARSE_RESIDENCY_BIT = 0x2,

    /// <summary> VK_IMAGE_CREATE_SPARSE_ALIASED_BIT specifies that the image will be backed using sparse memory binding with memory ranges that might also simultaneously be backing another image (or another portion of the same image). Images created with this flag must also be created with the VK_IMAGE_CREATE_SPARSE_BINDING_BIT flag. </summary>
    VK_IMAGE_CREATE_SPARSE_ALIASED_BIT = 0x4,

    /// <summary>
    ///     VK_IMAGE_CREATE_MUTABLE_FORMAT_BIT specifies that the image canbe used to create a VkImageView with a different format from the image. For
    ///     <a
    ...
    /// <summary> VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT specifies that the image canbe used to create a VkImageView of type VK_IMAGE_VIEW_TYPE_CUBE or VK_IMAGE_VIEW_TYPE_CUBE_ARRAY. </summary>
    VK_IMAGE_CREATE_CUBE_COMPATIBLE_BIT = 0x10,

    /// <summary>
    ///     VK_IMAGE_CREATE_ALIAS_BIT specifies that two images created with     the same creation parameters and aliased to the same memory can    interpret the contents of the memory consistently with each other,     subject to the rules described in the
    ///     <a href="https://registry.khronos.org/vulkan/specs/1.3-extensions/html/vkspec.html#resources-memory-aliasing"> Memory     Aliasing </a>
    ///     section.     This flag further specifies that each plane of a disjoint image can    share an in-memory non-linear representation with single-plane images,     and that a single-plane image can share an in-memory non-linear     representation with a plane of a multi-planar disjoint image, according     to the rules in
    ///     <a class="bare"
    ...
    /// <summary>
    ///     VK_IMAGE_CREATE_SPLIT_INSTANCE_BIND_REGIONS_BIT specifies that the image can be used with a non-zero value of the splitInstanceBindRegionCount member of a
    ///     <a
    ...
    /// <summary> VK_IMAGE_CREATE_2D_ARRAY_COMPATIBLE_BIT specifies that the image can be used to create a VkImageView of type VK_IMAGE_VIEW_TYPE_2D or VK_IMAGE_VIEW_TYPE_2D_ARRAY. </summary>
    VK_IMAGE_CREATE_2D_ARRAY_COMPATIBLE_BIT = 0x20,

    /// <summary> VK_IMAGE_CREATE_BLOCK_TEXEL_VIEW_COMPATIBLE_BIT specifies that the image having a compressed format can be used to create a VkImageView with an uncompressed format where each texel in the image view corresponds to a compressed texel block of the image. </summary>
    VK_IMAGE_CREATE_BLOCK_TEXEL_VIEW_COMPATIBLE_BIT = 0x80,

    /// <summary> VK_IMAGE_CREATE_EXTENDED_USAGE_BIT specifies that the image canbe created with usage flags that are not supported for the format the image is created with but are supported for at least one format a VkImageView created from the image can have. </summary>
    ...
    ///         multi-planar format
    ///     </a>
    ///     musthave each plane separately bound to memory, rather than having a single memory binding for the whole image; the presence of this bit distinguishes a disjoint image from an image without this bit set.
    /// </summary>
    VK_IMAGE_CREATE_DISJOINT_BIT = 0x200,

    /// <summary>
    ///     VK_IMAGE_CREATE_CORNER_SAMPLED_BIT_NV specifies that the image is a
    ///     <a href="https://registry.khronos.org/vulkan/specs/1.3-extensions/html/vkspec.html#resources-images-corner-sampled"> corner-sampled image </a>
    ///     .
    /// </summary>
    VK_IMAGE_CREATE_CORNER_SAMPLED_BIT_NV = 0x2000,

    /// <summary> VK_IMAGE_CREATE_SAMPLE_LOCATIONS_COMPATIBLE_DEPTH_BIT_EXTspecifies that an image with a depth or depth/stencil format can be used with custom sample locations when used as a depth/stencil attachment. </summary>
    ...
    ///     <br />
    ///     Image data read as an image sampler in the fragment shader will be additionally be read by the device during VK_PIPELINE_STAGE_VERTEX_SHADER_BIT if VkPhysicalDeviceFragmentDensityMap2PropertiesEXT::subsampledCoarseReconstructionEarlyAccessis VK_TRUE and the sampler was created with flagscontaining VK_SAMPLER_CREATE_SUBSAMPLED_COARSE_RECONSTRUCTION_BIT_EXT.
    ///     <br />
    ///     Image data read with load operations are resampled to the fragment density of the render pass if VkPhysicalDeviceFragmentDensityMap2PropertiesEXT::subsampledLoadsis VK_TRUE. Otherwise, values of image data are undefined.
    ///     <br />
    ...
    ///         multisampled rendering as a single-sampled framebuffer attachment
    ///     </a>
    /// </summary>
    ...
    /// <summary>
    ///     VK_IMAGE_CREATE_FRAGMENT_DENSITY_MAP_OFFSET_BIT_QCOM specifies that an image can be used in a render pass with non-zero
    ///     <a
    ///         href="https://registry.khronos.org/vulkan/specs/1.3-extensions/html/vkspec.html#renderpass-fragmentdensitymapoffsets">
    ///         fragment density map offsets
    ///     </a>
    ///     . In a renderpass with non-zero offsets, fragment density map attachments, input attachments, color attachments, depth/stencil attachment, resolve attachments, and preserve attachments must be created with VK_IMAGE_CREATE_FRAGMENT_DENSITY_MAP_OFFSET_BIT_QCOM.
    /// </summary>
