...
public unsafe struct VkBufferViewCreateInfo
{
    ...
    /// <summary>
    ///     offset is an offset in bytes from the base address of the buffer. Accesses to the buffer view from shaders use
    ///     addressing that is relative to this starting offset.
    /// </summary>
    public VkDeviceSize offset;

    /// <summary>
    ///     range is a size in bytes of the buffer view. If range is equal to VK_WHOLE_SIZE, the range from offset to the
    ///     end of the buffer is used. If VK_WHOLE_SIZE is used and the remaining size of the buffer is not a multiple of the
    ///     texel block size of format, the nearest smaller multiple is used.
    /// </summary>
