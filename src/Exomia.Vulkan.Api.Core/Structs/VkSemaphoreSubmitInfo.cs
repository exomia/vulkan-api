...
public unsafe struct VkSemaphoreSubmitInfo
{
    ...
    /// <summary> value is either the value used to signal semaphore or the value waited on by semaphore, if semaphore is a timeline semaphore. Otherwise it is ignored. </summary>
    public ulong value;

    /// <summary> stageMask is a VkPipelineStageFlags2 mask of pipeline stages which limit the first synchronization scope of a semaphore signal operation, or second synchronization scope of a semaphore wait operation as described in the semaphore wait operation and semaphore signal operation sections of the synchronization chapter. </summary>
    public VkPipelineStageFlags2 stageMask;

    /// <summary> deviceIndex is the index of the device within a device group that executes the semaphore wait or signal operation. </summary>
