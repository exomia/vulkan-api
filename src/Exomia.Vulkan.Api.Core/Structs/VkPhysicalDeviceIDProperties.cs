...
public unsafe struct VkPhysicalDeviceIDProperties
{
    ...
    /// <summary> deviceUUID is an array of VK_UUID_SIZE uint8_t values representing a universally unique identifier for the device. </summary>
    public fixed byte deviceUUID[(int)VK_UUID_SIZE];

    /// <summary> driverUUID is an array of VK_UUID_SIZE uint8_t values representing a universally unique identifier for the driver build in use by the device. </summary>
    ...
    /// <summary> deviceNodeMask is a uint32_t bitfield identifying the node within a linked device adapter corresponding to the device. </summary>
    public uint deviceNodeMask;

    /// <summary> deviceLUIDValid is a boolean value that will be VK_TRUE if deviceLUID contains a valid LUID and deviceNodeMask contains a valid node mask, and VK_FALSE if they do not. </summary>
